...
[ "$README_SIZE" -gt "$README_LOW" -a "$README_SIZE" -lt "$README_HIGH" ] && echo -e "${TERM_BGGREEN}README looks reasonable${TERM_RESET}" || (echo -e "${TERM_BGRED}README size looks off (${README_SIZE})${TERM_RESET}" && exit 1)
[ "$NOTICE_SIZE" -gt "$NOTICE_LOW" -a "$NOTICE_SIZE" -lt "$NOTICE_HIGH" ] && echo -e "${TERM_BGGREEN}NOTICE looks reasonable${TERM_RESET}" || (echo -e "${TERM_BGRED}NOTICE size looks off (${NOTICE_SIZE})${TERM_RESET}" && exit 1)
[ "$LICENSE_SIZE" -gt "$LICENSE_LOW" -a "$LICENSE_SIZE" -lt "$LICENSE_HIGH" ] && echo -e "${TERM_BGGREEN}LICENSE looks reasonable${TERM_RESET}" || (echo -e "${TERM_BGRED}LICENSE size looks off (${LICENSE_SIZE})${TERM_RESET}" && exit 1)
popd
echo done
